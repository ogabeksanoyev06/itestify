...
async function getResearchTest() {
   loading.value = true;
   try {
      const response = await testService.testTypes('research');
...
<template>
   <section class="py-8">
      ...
      <div class="grid gap-4 lg:grid-cols-3 sm:grid-cols-2 mt-8">
            ...
            >
               <div class="h-full flex flex-col justify-between p-2 sm:p-4 gap-2">
                  <div class="flex flex-col gap-2">
                     ...
                     <h3 class="font-semibold text-base leading-none tracking-tight line-clamp-2">{{ item.title }}</h3>
                     <p class="text-sm text-muted-foreground">
                        {{ item.short_description }}
                     </p>
                  </div>
                  <div
                     class="flex items-center justify-center flex-col space-y-2 sm:flex-row sm:justify-between sm:space-x-2"
                  >
                     <Button class="w-full sm:w-auto" @click="goToResearchTest(2)">Testni boshlash</Button>
                     <Button variant="outline" class="w-full sm:w-auto">Sotib olish</Button>
                  </div>
               </div>

               <div class="absolute top-0 right-0">
                  <Badge variant="secondary" class="text-blue-500">{{ item.price }} so'm</Badge>
               </div>
            </div>
         </div>
      </div>
