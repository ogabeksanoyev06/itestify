   ...
   <div class="py-8">
      <div class="container">
         <div class="flex flex-wrap justify-center sm:justify-start gap-2 mb-8">
            <Button
               :variant="item.id === activeCategory ? 'default' : 'outline'"
            ...
            <nuxt-link v-for="(item, index) in courses" :key="index" :to="`/courses/${item.id}`">
               <div class="group relative flex h-full flex-col overflow-hidden rounded-md border transition hover:bg-muted/30 cursor-pointer">
                  ...
                  <div class="space-y-3 p-3">
                     <div class="line-clamp-2 text-base font-medium transition group-hover:text-primary md:text-xl">
                        {{ item.title }}
                     </div>
                     <div class="flex items-center gap-5 text-sm flex-wrap">
                        <div class="flex items-center space-x-2">
                           ...
                           <span>{{ item.modules_count }} ta modul</span>
                        </div>
                        <div class="flex items-center space-x-2">
                           <svg
                              stroke="currentColor"
                           ...
                           <span>{{ item.lessons_count }} ta video</span>
                        </div>
                     </div>
                     <div class="flex justify-between items-center flex-wrap">
                        <div class="grid">
                           <div class="flex items-center space-x-1">
                              <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4">
                                 <path
                              ...
                              <span class="text-sm">43 min</span>
                           </div>
                           <div class="flex items-center space-x-1">
                              <svg viewBox="0 0 24 24" width="24" height="24" stroke="#16a34a" stroke-width="3" class="icon h-4 w-4">
                                 <line x1="6" y1="15" x2="6" y2="19"></line>
                                 <line stroke="" x1="12" y1="10" x2="12" y2="19"></line>
                                 <line stroke="rgba(0, 0, 0, 0.2)" x1="18" y1="5" x2="18" y2="19"></line>
                              </svg>
                     ...
                     <p class="text-base font-medium text-slate-700 dark:text-slate-200">{{ currencyFormat(item.price) }} so‘m</p>
                  </div>
                  <div class="absolute"><Badge>Bepul</Badge></div>
               </div>
            </nuxt-link>
         </div>
      </div>
